...
COMPANIES_FILE = sys.argv[4] # '/data/1_raw/companies-scrapy.csv'
OUTPUT_FILE = sys.argv[5]  # 'data/2_staging/metrics/dataset_na_comp_targetrank/'
...
# Leemos el fichero companies (CSV)
companiesDF = spark.read.csv(BUCKET + COMPANIES_FILE, header=True, inferSchema=True, sep='|')
companiesDF = companiesDF.select(F.col('CODE'), F.col('INDUSTRY01'), F.col('INDUSTRY02'))

# Join con companies
TotalDF = TotalDF.join(companiesDF, on=['CODE'])
